...
public class MemberService {
    ...
    public boolean signUp(SignupRequest request) {
        ...
        if (isJoinedUser(request)) {
            throw new MemberServiceException(HttpStatus.BAD_REQUEST, "Already joined member.");
        }
        // id 중복 확인
    ...
    public Member withdraw(String userId) {
        var user = this.memberRepository.findById(userId).get();
        user.withdraw();
    ...
    public Member cancelWithdraw(CancelWithdrawRequest request) {
        var user = this.memberRepository.findByUserId(request.userId());
        if (user.getWithdrawalDateTime() != null && user.isAbleToCancelWithdrawal()) {
            user.cancelWithdraw();
        }
        return this.memberRepository.save(user);
    }
